...
## Dotenv / TypeScript infrastructure checks — 2026-09-21

Optional root `.env` now configures local API/Prisma/seed/rename commands without shell exports; explicit environment wins. See [configuration](ENVIRONMENT_CONFIGURATION.md). Cloud/packaged/production/test contexts retain injected configuration. Run `pnpm test:infrastructure` for the custom TypeScript policy suite. The native [Terraform candidate](../../infrastructure/terraform/README.md) supports credential-free mocked verification alongside preserved CloudFormation/Python history.
