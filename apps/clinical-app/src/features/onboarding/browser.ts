...
export const onboardingBrowser = {
  ...
  consume() { history.replaceState(null, "", location.pathname + location.search); },
  subscribe(listener: () => void) {
    window.addEventListener("hashchange", listener);
    return () => window.removeEventListener("hashchange", listener);
  },
  reveal() {
    window.requestAnimationFrame(() => document.getElementById("account-onboarding-panel")?.scrollIntoView({ behavior: "smooth", block: "start" }));
  },
};
