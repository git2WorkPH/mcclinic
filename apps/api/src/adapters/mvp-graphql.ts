...
export function installMvpGraphql(
  app: Express,
  database: Database,
  delivery?: Delivery,
  ...
  const resolvers: Resolvers<Context> = {
    Query: {
      accountMfaEnabled: (_p, _a, c) =>
        safe(c, 'account.security', () => onboarding.status(c.actor)),
      practices: (_p, _a, c) =>
    ...
    Mutation: {
      registerAccount: (_p, a, c) =>
        safe(c, 'account.register', async () => {
          await onboarding.register(a.input);
          return true;
        }),
      resendAccountVerification: (_p, a, c) =>
        safe(c, 'account.verify.request', async () => {
          await onboarding.resend(a.email);
          return true;
        }),
      verifyAccount: (_p, a, c) =>
        safe(c, 'account.verify', async () => {
          await onboarding.verify(a.token);
          return true;
        }),
      requestPasswordReset: (_p, a, c) =>
        safe(c, 'account.reset.request', async () => {
          await onboarding.resetRequest(a.email);
          return true;
        }),
      resetAccountPassword: (_p, a, c) =>
        safe(c, 'account.password.reset', async () => {
      ...
      invitePracticeMember: (_p, a, c) =>
        safe(c, 'membership.invite', async () => {
          await onboarding.invite(c.actor, a.email, a.role);
          return true;
        }),
      acceptPracticeInvitation: (_p, a, c) =>
        safe(c, 'membership.accept', async () => {
      ...
      startAccountMfa: (_p, a, c) =>
        safe(c, 'account.mfa.enroll', async () =>
          JSON.stringify(
            await onboarding.startMfa(
              c.actor,
              z.string().max(256).parse(a.password),
            ),
          ),
        ),
      confirmAccountMfa: (_p, a, c) =>
        safe(c, 'account.mfa.enabled', async () =>
          JSON.stringify(
            await onboarding.confirmMfa(
              c.actor,
              z.string().max(64).parse(a.code),
            ),
          ),
        ),
      disableAccountMfa: (_p, a, c) =>
        safe(c, 'account.mfa.disabled', async () => {
          await onboarding.disableMfa(
            c.actor,
            z.string().max(256).parse(a.password),
            z.string().max(64).parse(a.code),
          );
      ...
      login: (_p, a, c) =>
        safe(c, 'session.login', async () => {
          ...
          const result = await services.identity.login(
            username,
            password,
            z
              .string()
