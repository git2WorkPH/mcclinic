import { configuredKeyring } from './keyring.js';
...
export function totp(secret: string, step: number, digits = 6) {
  ...
  return String((h.readUInt32BE(offset) & 0x7fffffff) % 10 ** digits).padStart(
    digits,
    '0',
  );
}
...
export function localKey() {
  const dir = localStateDirectory();
  mkdirSync(dir, { recursive: true, mode: 0o700 });
  const path = resolve(dir, 'onboarding-key');
  try {
  ...
  if (key.length !== 32) throw new Error('Invalid onboarding encryption key.');
  return key;
}
export function encrypt(value: string) {
  const ring = configuredKeyring();
  if (ring) return ring.encrypt(value);
  const iv = randomBytes(12),
    cipher = createCipheriv('aes-256-gcm', localKey(), iv);
  const result = Buffer.concat([cipher.update(value, 'utf8'), cipher.final()]);
  return [
    iv.toString('base64'),
...
export function decrypt(value: string) {
  if (value.startsWith('v1.')) {
    const ring = configuredKeyring();
    if (!ring) throw new Error('Identity keyring unavailable.');
    return ring.decrypt(value);
  }
