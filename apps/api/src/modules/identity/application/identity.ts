...
export interface IdentityPorts {
  ...
  failed(): Promise<void>;
  limitLogin?(username: string): Promise<void>;
}
export function identityUseCases(ports: IdentityPorts) {
  return {
    async login(
      ...
      password: string,
      code = '',
      persistentLimit = false,
    ) {
      if (persistentLimit) {
        if (!ports.limitLogin)
          throw new AppError('UNAUTHENTICATED', 'Login limit unavailable.');
        await ports.limitLogin(username.trim().toLowerCase());
      }
      ...
      const actor: Actor = {
        ...
        name: account.name,
        role: account.role,
        ...(account.credentialVersion !== undefined
    ...
    resolve(token: string) {
      return ports.resolve(token);
    },
    logout(token: string) {
      return ports.revoke(token);
    },
  };
}
